...
var poision_scene = preload("../PoisionMesh.tscn")
...
func seek() -> Vector3:
    ...
    if target != null:
        var desired = (target - global_transform.origin).normalized() * speed
        # if (desired - vel).normalized().length() > 0.1:
        steer = (desired - vel).normalized() * steer_force
    return steer

func update_speed() -> float:
    if (target != null) and global_transform[3].length() < self.current_max_distance(target):
        speed_factor = global_transform[3].length() / self.current_max_distance(target)
